...
// usage in command line: root -l "convert_to_csv.cc(/"run/",/"tagnr/",/"event"/)
// where event = directory where the /run_01 etc directory is
// run = runnumber (previous line it is 01)
// tagnr = tag in delphes file
// note that the file is called tag_x_delphes_events.root
// the output will be on screen, so pipe to file
// if file has different name, modify fullpath lines 40-47.
// file will create csv file suitable for training, variable sizes of lines and objects
...
using namespace std;
   ...
   const Int_t kMaxParticle = 13792;
   const Int_t kMaxTrack = 1198;
   const Int_t kMaxTower = 1542;
   const Int_t kMaxEFlowTrack = 1198;
   const Int_t kMaxEFlowPhoton = 1334;
   const Int_t kMaxEFlowNeutralHadron = 1180;
   const Int_t kMaxGenJet = 113;
   const Int_t kMaxGenMissingET = 11;
   const Int_t kMaxJet = 111;
   const Int_t kMaxElectron = 12;
   const Int_t kMaxPhoton = 13;
   const Int_t kMaxMuon = 12;
   const Int_t kMaxMissingET = 11;
   const Int_t kMaxScalarHT = 11;
...
// start of event loop, main part of the code
for (Long64_t jentry = 0; jentry < fChain->GetEntries(); jentry++)
	{	
		...
		if(*Event_Weight < 0) continue;
		if(Particle_size == 0) continue;
		if(*MissingET_MET > 0.) 
			{
				accept_event = 1;
				double Meff = *MissingET_MET ;
				for (int objects = 0; objects < Electron_size; objects ++) Meff = Meff + Electron_PT[objects];
				for (int objects = 0; objects < Muon_size; objects ++) Meff = Meff + Muon_PT[objects];
				for (int objects = 0; objects < Jet_size; objects ++) Meff = Meff + Jet_PT[objects];
				for (int objects = 0; objects < Photon_size; objects ++) Meff = Meff + Photon_PT[objects];
			}
		else if (Electron_size > 0)
			{
				lead_lep = Electron_PT[0];
				for (int objects = 0; objects < Electron_size; objects ++)
				{
					if (Electron_PT[objects] > 25 && (( abs(Electron_Eta[objects]) > 0 && abs(Electron_Eta[objects]) < 1.37) || (abs(Electron_Eta[objects]) > 1.52 && abs(Electron_Eta[objects]) < 2.47))) 
					{
						accept_event = 1;
						break;
					}
			...
			{
				if(Muon_PT[0] > lead_lep) lead_lep = Muon_PT[0];
				for (int objects = 0; objects < Muon_size; objects ++)
				{
					if (Muon_PT[objects] > 25 && abs(Muon_Eta[objects]) < 2.7) 
					{
						accept_event = 1;
						break;
					}
		...
		else if (Photon_size > 0)
			{
			for (int objects = 0; objects < Photon_size; objects ++)
				{
				if (Photon_PT[objects] > 25 && abs(Photon_Eta[objects]) < 2.37)
					{
					accept_event = 1;
					break;
					}
		...
		else if (Jet_size > 0) 
			{
				...
				for (int objects = 0; objects < Jet_size; objects ++)
				{
					if (Jet_PT[objects] > 60 && abs(Jet_Eta[objects]) < 2.8)
					{
						accept_event = 1;
						break;
					}
...
//note that the energies are in MeV
		if(accept_event == 1)
		{
			cout << *Event_Weight << ";" << *MissingET_MET*1000. << ";" << *MissingET_Phi << ";";
			for (int objects = 0; objects < Jet_size; objects ++)
					{
						if (Jet_BTag[objects] == 0)cout << "j," ;
						else cout << "b,";
						cout << Jet_PT[objects]*1000. << "," << Jet_Eta[objects] << "," << Jet_Phi[objects] << ";";
					}		
			for (int objects = 0; objects < Electron_size; objects ++)
					{
						if (Electron_Charge[objects] == 1)cout << "e+," ;
						else cout << "e-,";
						cout << Electron_PT[objects]*1000. << "," << Electron_Eta[objects] << "," << Electron_Phi[objects] << ";";
					}		
			for (int objects = 0; objects < Muon_size; objects ++)
					{
						if (Muon_Charge[objects] == 1)cout << "m+," ;
						else cout << "m-,";
						cout << Muon_PT[objects]*1000. << "," << Muon_Eta[objects] << "," << Muon_Phi[objects] << ";";
					}
			for (int objects = 0; objects < Photon_size; objects ++)
					{
